...
- **`#` is a comment delimiter everywhere in Klipper macros — even inside string literals in `{ ... }` action blocks, and even inside multi-line jinja `{# ... #}` comment blocks.** A `#` in `{ action_raise_error("...#TBD...") }` truncates the string and breaks the template. Worse: a multi-line `{# foo #}` block where the opening `{#` and closing `#}` are on different lines gets eaten by Klipper's config parser (which strips `#`-to-EOL line-by-line BEFORE jinja sees the body), leaving jinja with `{` followed by raw text — typically failing on the next apostrophe (e.g. `don't`). Workaround: keep `{# ... #}` blocks single-line, or use Klipper `# ...` line comments outside the `{# #}` block (those get stripped cleanly per line). Single-line `{# tag #}` works fine. (Caught by PR #18 for the string-literal case after the Eddy migration deploy; caught again by PR #73 CI for the multi-line jinja case during the chamber control review-fixup pass.)
