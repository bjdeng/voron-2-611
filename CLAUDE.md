...
- `PRINT_START` — full start: tap_threshold guard → param validation (BED/EXTRUDER max-temp) → CLEAR_PAUSE + UI hints (M117, SET_PRINT_STATS_INFO TOTAL_LAYER) → bed + hotend partial heat NON-BLOCKING → home + QGL (cold, in parallel with heat) → wait for bed + hotend partial → chamber soak branch (`CHAMBER>0` from slicer; `CHAMBER=0` runs a `bed_stabilization_soak_seconds` G4) → first tap-Z (partial-warm; gives adaptive mesh its Z reference) → optional `Z_ADJUST` → adaptive bed mesh → final `M109` → hot-nozzle `BLOBIFIER_CLEAN` → **second tap-Z (hot + clean nozzle, multi-sample retry-until-consensus)**. The two-tap pattern restores the eddy-ng pre-migration accuracy: `SET_Z_FROM_PROBE` passes `SAMPLES=3 SAMPLES_TOLERANCE=0.020 SAMPLES_TOLERANCE_RETRIES=2` so the eddy probe code (which IGNORES `samples:` config block for tap mode — see `EddyParameterHelper.get_probe_params` at `vendor/klipper/klippy/extras/probe_eddy_current.py:1014-1036`) gets the multi-sample retry behavior via gcmd args. Spec: `docs/superpowers/specs/2026-05-18-print-lifecycle-redesign.md`. Slicer contract: `docs/slicer-templates/orcaslicer.md`.
...
- **Microsteps 64 on X/Y/Z mainboard steppers** (was 128 from prior forum advice; halved to 64 on 2026-05-20 via PR #95, Phase A of [#24](https://github.com/bjdeng/voron-2-611/issues/24)). `interpolate: False` retained. Position drift 0 mm and step_compress count 0 confirmed via TEST_SPEED; recovered ~50% of the LPC1769 USB step-rate budget at max_velocity. **Empirical caveat:** Klipper docs (TMC_Drivers.md:106-109) predicted 64 = 128 acoustically, but on this machine post-change TEST_SPEED was rated 3.5/5 vs 3.0/5 baseline (slightly louder, "maybe more vibration noise", no abnormal sounds). Phase B (TMC Autotune) is in flight to compensate; re-evaluation pending after Phase B lands.
