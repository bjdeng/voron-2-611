...
- **Heater cartridge: 24 V 40 W**, 6 mm × 20 mm, on EBB HE0 (`heater_pin: EBB:gpio7`). Replaced 2026-06-03 after the original failed open-circuit ~36 min into an ASA print (PWM pegged at 100% with low melt load, smooth thermal decay → `verify_heater` shutdown). The 40 W rating caps sustainable volumetric flow; a 50 W upgrade is planned once tuning/filament calibration is complete — see [#125](https://github.com/bjdeng/voron-2-611/issues/125). Re-run `PID_CALIBRATE HEATER=extruder` on any cartridge swap.
